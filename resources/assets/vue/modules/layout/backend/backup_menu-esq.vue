    ...
    >
        ...
        <v-toolbar flat class="transparent">
            <v-list class="pa-0">
                <v-list-tile>
                    <v-list-tile>
                        <a href="/admin">
                            <img src="/img/ayro.png" style="max-width:60px">
                        </a>
                    </v-list-tile>
                    <v-list-tile-content>
                        <v-list-tile-title><span>Akhena</span></v-list-tile-title>
                    </v-list-tile-content>
                    <v-list-tile-action>
        ...
        <v-list dense>
            <template v-for="item in itens"
                ...
                >
                    <v-list-tile slot="activator" v-show="!item.permission  || $can(item.permission ? item.permission : 'public')"
                                 active-class="secondary--text"
                                 :value="temFilhoAtivo(item)"

                    >
                        <v-list-tile-content active-class="secondary--text">
                            <v-list-tile-title v-if="item.ativo" active-class="secondary--text">
                                <strong >{{ item.nome }}</strong>
                            </v-list-tile-title>
                            <v-list-tile-title v-else active-class="secondary--text">
                                {{ item.nome }}
                            </v-list-tile-title>
                        </v-list-tile-content>
                    </v-list-tile>

                    <v-list-tile v-show="!subItem.permission  || $can(subItem.permission ? subItem.permission : 'public')"
...
//            bus.$on('set-menu-active', (value) => self.ajustaMenuAtual(value));
        ...
        methods: {
            ...
            temFilhoAtivo: function (itemAtual){
                // var self = this;
            ...
            ajustaMenuAtual: function (value) {
                var tmp = value;
                // desabilita todos
                for (var k in this.dataReturn) {
                    this.dataReturn[k].ativo = false;
                }
                for (var k in this.dataReturn) {
                    // atualiza o menu ativo
                    if (this.dataReturn[k].slug == tmp) {
                        this.dataReturn[k].ativo = true;
                    }
                }
