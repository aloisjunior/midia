<template>
    <material-card
            ...
            :title="getTitulo('Cadastro de Usuários')"
    >
                    ...
                    <v-flex xs12
                        ...
                        >
                            <v-flex xs12 text-xs-center>
                                <h3 class="font-weight-light py-1">
                                    Os dados da sua consulta aparecerão aqui.<br>{{$can('usuario-store')? 'Você também pode incluir novo registro clicando abaixo.' : ''}}<br>
                                </h3>
                    ...
                    <v-flex xs12
                        ...
                        >
                            ...
                            <template slot="items" slot-scope="props">
                                ...
                                <td class="text-xs-center hidden-md-and-down">{{ props.item.created_at | formatDate }}</td>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                ...
                headers: [
                    ...
                    {text: 'Criação', value: 'created_at', align: 'center', class: 'hidden-md-and-down'},
        ...
        mounted() {
            ...
            bus.$on('usuario:updated', () => self.loadDados());
...
//            this.loadDados();
        ...
        methods: {
            openViewAdd: function (item) {
                this.$router.push({name:'usuarios-add'});
            },
            openViewEdit: function (item) {
                this.$router.push({name:'usuarios-edit', params: { id: item.id }});
            },
            loadDados: function () {
                if (this.loading) return false;
                let self = this;
                this.loading = true;
                this.$http.post('/admin/usuario/listar', this.pesquisa)
                    .then(response => {
//                        console.log(response);
            ...
            excluirRegistro: function () {
                ...
                this.loading = true;
                this.$http.get('/admin/usuario/' + self.itemSelected.id + '/destroy')
                    .then(response => {
                        self.loading = false;
