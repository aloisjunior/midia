<template>
    <material-card
            ...
            :title="getTitulo('Cadastro de Permissões')"
    >
...
<div class="font-weight-light subheading pt-1">{{$can('permissao-store')? 'Para novo registro clique abaixo.' : ''}}</div>
...
<script>
    ...
    export default {
        ...
        name: 'aton-admin-permissoes-view',
        data() {
            return {
                ...
                headers: [
                    ...
                    {text: 'Nome', value: 'permission_title', align: 'left'},
                    {text: 'Slug', value: 'permission_slug', align: 'left'},
                    {text: 'Descrição', value: 'permission_description', align: 'left'},
        ...
        mounted() {
            ...
            bus.$on('permissao:updated', () => self.loadDados());
...
//            this.loadDados();
        ...
        methods: {
            openViewAdd: function (item) {
                this.$router.push({name:'permissoes-add'});
            },
            openViewEdit: function (item) {
                this.$router.push({name:'permissoes-edit', params: { id: item.id }});
            },
            loadDados: function () {
                if (this.loading) return false;
                let self = this;
                this.loading = true;
                this.$http.post('/admin/permissao/listar', this.pesquisa)
                    .then(response => {
//                        console.log(response);
            ...
            excluirRegistro: function () {
                ...
                this.loading = true;
                this.$http.get('/admin/permissao/' + self.itemSelected.id + '/destroy')
                    .then(response => {
                        self.loading = false;
