<template>
    <material-card
            ...
            :title="getTitulo('Cadastro de Estações')"
    >
...
<div class="font-weight-light subheading pt-1">{{$can('estacao-store')? 'Para novo registro clique abaixo.' : ''}}</div>
                    ...
                    <v-flex xs12
                        ...
                        >
                            ...
                            <template slot="items" slot-scope="props">
                                ...
                                <td class=" text-xs-center">{{ props.item.porta }}</td>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                ...
                headers: [
                    ...
                    {text: 'Porta', value: 'posta', align: 'center'},
        ...
        mounted() {
            ...
            bus.$on('estacao:updated', () => self.loadDados());
...
//            this.loadDados();
        ...
        methods: {
            openViewAdd: function (item) {
                this.$router.push({name:'estacoes-add'});
            },
            openViewEdit: function (item) {
                this.$router.push({name:'estacoes-edit', params: { id: item.id }});
            },
            loadDados: function () {
                if (this.loading) return false;
                let self = this;
                this.loading = true;
                this.$http.post('/admin/estacao/listar', this.pesquisa)
                    .then(response => {
//                        console.log(response);
            ...
            excluirRegistro: function () {
                ...
                this.loading = true;
                this.$http.get('/admin/estacao/' + self.itemSelected.id + '/destroy')
                    .then(response => {
                        self.loading = false;
