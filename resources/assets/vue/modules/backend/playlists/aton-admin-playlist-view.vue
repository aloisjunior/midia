<template>
    <material-card
            ...
            :title="getTitulo('Cadastro de Playlists')"
    >
...
<div class="font-weight-light subheading pt-1">{{$can('playlist-store')? 'Para novo registro clique abaixo.' : ''}}</div>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                ...
                headers: [
                    ...
                    {text: 'Duração', value: 'tempo', align: 'center'},
        ...
        mounted() {
            ...
            bus.$on('playlist:updated', () => self.loadDados());

        },
        methods: {
            openViewAdd: function (item) {
                this.$router.push({name: 'playlists-add'});
            },
            openViewEdit: function (item) {
                this.$router.push({name: 'playlists-edit', params: {id: item.id}});
            },
            loadDados: function () {
...
//                        console.log(response);
            ...
            excluirRegistro: function () {
                ...
                this.loading = true;
                this.$http.get('/admin/playlist/' + self.itemSelected.id + '/destroy')
                    .then(response => {
                        self.loading = false;
