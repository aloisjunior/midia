<template>
    <material-card
            ...
            :title="getTitulo('Cadastro de Arquivos do Sistema')"
    >
...
<div class="font-weight-light subheading pt-1">{{$can('arquivo-store')? 'Para novo registro clique abaixo.' : ''}}</div>
                    ...
                    <v-flex xs12
                        ...
                        >
                            <template slot="items" slot-scope="props">
                                ...
                                <td class=" text-xs-left caption">
                                    <a target="_blank" :href="props.item.link_externo">
                                        {{ props.item.link_externo }}
                                    </a>
                                </td>
                                <td class=" text-xs-center">{{ props.item.mime }}</td>
                                <td class=" text-xs-center">
                                    <v-icon  small  v-if="props.item.ativo">check_box</v-icon>
                                    <v-icon v-else  small  >check_box_outline_blank</v-icon>
                                </td>
                                <td class=" text-xs-center">
                                    <v-icon  small  v-if="props.item.exibe_site">check_box</v-icon>
                                    <v-icon v-else  small  >check_box_outline_blank</v-icon>
                                </td>
                                    ...
                                    <v-layout row fill-height justify-end align-center>
                                        <v-tooltip content-class="top" top  >
                                            <v-btn  icon small  class="mx-0"
                                                    @click="callDownload(props.item)"
                                                    slot="activator"
                                                    v-show="$can('arquivo-update')"
                                            >
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                ...
                headers: [
                    ...
                    {text: 'Nome | Descrição', value: 'nome', align: 'left', width: '20%'},
                    {text: 'Link', value: 'link_externo', align: 'left'},
                    {text: 'Tipo', value: 'mime', align: 'center'},
                    {text: 'Ativo', value: 'ativo', align: 'center'},
                    {text: 'Exibe Site', value: 'exibe_site', align: 'center'},
        ...
        mounted() {
            ...
            bus.$on('arquivo:updated', () => self.loadDados());
...
//            this.loadDados();
        ...
        methods: {
            ...
            openViewAdd: function (item) {
                this.$router.push({name:'arquivos-add'});
            },
            openViewEdit: function (item) {
                this.$router.push({name:'arquivos-edit', params: { id: item.id }});
            },
            loadDados: function () {
                if (this.loading) return false;
                let self = this;
                this.loading = true;
                this.$http.post('/admin/arquivo/listar', this.pesquisa)
                    .then(response => {
//                        console.log(response);
            ...
            excluirRegistro: function () {
                ...
                this.loading = true;
                this.$http.get('/admin/arquivo/' + self.itemSelected.id + '/destroy')
                    .then(response => {
                        self.loading = false;
